...
function nexusDeploy {
	...
	echo "****"	
	echo "Deploying the jar /highcharts-1.0.jar as ${ArtifactId}${rstate}.jar to Nexus...."
	echo "****"	
    ...
    mvn deploy:deploy-file \
        ...
        -Dfile=target/highcharts-1.0.jar
...
if [ $rsp == 0 ]; then
  ...
  zip highcharts_$rstate.zip install/* target/*.jar
  cp highcharts_$rstate.zip /home/$USER/eniq_events_releases

  nexusDeploy

  \rm -rf highcharts_$rstate.zip
  \rm -rf install
fi
